...
objName ==objName_before ):
    ...
    def draw_canvas_with_pull(self, rrv_x, datahist, mplot, mplot_pull,ndof,parameters_list,in_directory, in_file_name, in_model_name="", show_constant_parameter=0, logy=0,ismj=0, fix_axis=0, force_plots=0):# mplot + pull
        ...
        if 'm_j_prefit' in in_file_name:
            medianLine_sb_lo = TLine(mplot.GetXaxis().GetXmin(),0.,65.,0.); medianLine_sb_lo.SetLineWidth(2); medianLine_sb_lo.SetLineColor(kBlack); medianLine_sb_lo.Draw();
            medianLine_sb_hi = TLine(105,0.,mplot.GetXaxis().GetXmax(),0.); medianLine_sb_hi.SetLineWidth(2); medianLine_sb_hi.SetLineColor(kBlack); medianLine_sb_hi.Draw();
        else:
            medianLine = TLine(mplot.GetXaxis().GetXmin(),0.,mplot.GetXaxis().GetXmax(),0); medianLine.SetLineWidth(2); medianLine.SetLineColor(kBlack); medianLine.Draw();
        mplot_pull.Draw("Psame")
        ...
        pt = TPaveText(0.2,0.72,0.475,0.89, "blNDC")
        pt.SetFillStyle(0)
        pt.SetBorderSize(0)
        pt.SetTextAlign(12)
    ...
    def fit_mlvj_model_single_MC(self,in_file_name, label, in_range, mlvj_model, deco=0, show_constant_parameter=0, logy=0, ismc=0):
        ...
        ## Colours for model and points
        colour = 0
        if 'TTbar' in in_file_name:
            colour = self.color_palet['TTbar']
        elif 'WJets' in in_file_name and 'sb_WJets0' not in in_file_name:
            colour = self.color_palet['WJets']-1
        elif 'STop' in in_file_name:
            colour = self.color_palet['STop']
        elif 'WW' in in_file_name:
            colour = self.color_palet['WW']
        elif 'WZ' in in_file_name:
            colour = self.color_palet['WW']
        ...
        draw_error_band_extendPdf(rdataset, model, rfresult,mplot,colour,"L")
        model.plotOn( mplot, RooFit.LineColor(colour+2) )#, RooFit.VLines()); in order to have the right pull 
        rdataset.plotOn( mplot , RooFit.MarkerSize(1), RooFit.DataError(RooAbsData.SumW2), RooFit.XErrorSize(0), RooFit.MarkerColor(colour+4), RooFit.LineColor(colour+4) );

        ## Add legend
        leg = TLegend(0.66,0.58,0.91,0.88)
        leg.SetFillColor(0)
        leg.SetFillStyle(0)
        leg.SetBorderSize(0)
        leg.SetLineColor(0)
        ...
        leg.SetLineStyle(0)
        leg.SetTextFont(42)
        leg.AddEntry(mplot.getObject(4), "MC data","PE")
        leg.AddEntry(mplot.getObject(3), "Fitted model","L")
        leg.AddEntry(mplot.getObject(2), "#pm 1#sigma uncertainties","L")
        mplot.addObject(leg)
        ...
        mplot.GetYaxis().SetRangeUser(1e-2,mplot.GetMaximum()*1.2);
        mplot_pull.SetMarkerColor(colour+4);
        mplot_pull.SetLineColor(colour+4);
	...
	if 'TTbar' in in_file_name:
        ...
        draw_error_band_extendPdf(rdataset_mj, model, rfresult,mplot,colour,"L");
        ## draw the function
        model.plotOn( mplot, RooFit.LineColor(colour+2) );# remove RooFit.VLines() in order to get right pull in the 1st bin
	## re-draw the dataset
        rdataset_mj.plotOn( mplot , RooFit.MarkerSize(1), RooFit.DataError(RooAbsData.SumW2), RooFit.XErrorSize(0), RooFit.MarkerColor(colour+4), RooFit.LineColor(colour+4) );
